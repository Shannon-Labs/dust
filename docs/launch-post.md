...
Dust exists to collapse that loop into one local binary with explicit state.
...
The more workflows are driven by agents, tests, and small scripts, the less patience there is for container boot sequences and hand-wired database glue. The useful product is the local runtime that starts instantly and keeps state legible.

## What Makes It Different

- One binary instead of Docker plus helper scripts
