...
run_dust_bench() {
    ...
    # Always use an absolute path for the binary
    local DUST="$PROJECT_ROOT/target/release/dust"
    if [ ! -x "$DUST" ]; then
        echo "Building dust release..."
        (cd "$PROJECT_ROOT" && cargo build --release -p dust-cli 2>/dev/null)
    fi
    ...
    echo -n "  init: "
    INIT_START=$(date +%s%N)
    $DUST init --force "$WORK" >/dev/null 2>&1
    INIT_END=$(date +%s%N)
    ...
    (cd "$WORK" && $DUST query "$SCHEMA_SQL" >/dev/null 2>&1)
    SCHEMA_END=$(date +%s%N)
    ...
    (cd "$WORK" && $DUST query "$INSERT_SQL" >/dev/null 2>&1)
    INSERT_END=$(date +%s%N)
    ...
    for i in $(seq 1 100); do
        (cd "$WORK" && $DUST query "SELECT * FROM bench WHERE id = $i" >/dev/null 2>&1)
    done
    QUERY_END=$(date +%s%N)
    ...
    # 5. Aggregate query
    echo -n "  aggregate (COUNT+SUM, 1000 rows): "
    AGG_START=$(date +%s%N)
    (cd "$WORK" && $DUST query "SELECT count(*), sum(score) FROM bench" >/dev/null 2>&1)
    AGG_END=$(date +%s%N)
    ...
    WIN_START=$(date +%s%N)
    (cd "$WORK" && $DUST query "SELECT name, ROW_NUMBER() OVER (ORDER BY score DESC) FROM bench" >/dev/null 2>&1)
    WIN_END=$(date +%s%N)
    ...
    (cd "$WORK" && $DUST branch create test-branch >/dev/null 2>&1)
    BRANCH_END=$(date +%s%N)
    ...
    cat <<EOF > "$RESULT_FILE"
{
  ...
  "dust": {
    ...
    "100_point_queries_total_ms": $QUERY_MS,
...
run_sqlite_bench() {
    ...
    SCHEMA_END=$(date +%s%N)
    SQLITE_SCHEMA_MS=$(( (SCHEMA_END - SCHEMA_START) / 1000000 ))
    echo "${SQLITE_SCHEMA_MS}ms"
    ...
    INSERT_END=$(date +%s%N)
    SQLITE_INSERT_MS=$(( (INSERT_END - INSERT_START) / 1000000 ))
    echo "${SQLITE_INSERT_MS}ms"
    ...
    QUERY_END=$(date +%s%N)
    SQLITE_QUERY_MS=$(( (QUERY_END - QUERY_START) / 1000000 ))
    SQLITE_QUERY_AVG=$(( SQLITE_QUERY_MS / 100 ))
    echo "${SQLITE_QUERY_MS}ms total (${SQLITE_QUERY_AVG}ms/query)"

    # 4. Aggregate
    echo -n "  aggregate (COUNT+SUM, 1000 rows): "
    AGG_START=$(date +%s%N)
    $SQLITE "$DB" "SELECT count(*), sum(score) FROM bench" >/dev/null
    AGG_END=$(date +%s%N)
    SQLITE_AGG_MS=$(( (AGG_END - AGG_START) / 1000000 ))
    echo "${SQLITE_AGG_MS}ms"
    ...
    WIN_END=$(date +%s%N)
    SQLITE_WIN_MS=$(( (WIN_END - WIN_START) / 1000000 ))
    echo "${SQLITE_WIN_MS}ms"
...
with open('$RESULT_FILE', 'r') as f:
    data = json.load(f)
data['sqlite'] = {
    'create_50_tables_ms': $SQLITE_SCHEMA_MS,
    'insert_1000_rows_ms': $SQLITE_INSERT_MS,
    '100_point_queries_total_ms': $SQLITE_QUERY_MS,
    'avg_point_query_ms': $SQLITE_QUERY_AVG,
    'aggregate_ms': $SQLITE_AGG_MS,
    'window_function_1000_ms': $SQLITE_WIN_MS
}
