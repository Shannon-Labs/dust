...
fn test_full_workflow() {
    ...
    assert!(
        !diff["table_diffs"].as_array().unwrap().is_empty(),
        "should have diffs: {diff}"
    );
    ...
    // Column name is count(...) in dust's output; emitted as a JSON string
    assert_eq!(
        counts[0]["count(...)"], "3",
        "main should still have 3 rows"
    );
...
fn test_error_handling() {
    ...
    assert!(
        ...
        "unknown method should return error: {resp}"
    );
...
fn test_resources() {
    ...
    let resp = session.request(
        "resources/read",
        Some(json!({"uri": "dust://branch/current"})),
    );
